...
<body>
	<div class="container">
		...
		<form class="form-horizontal" method="post"
			action="${pageContext.request.contextPath}/prof_edit_ok.do">
				...
				<div class="col-sm-10">
					<select name="deptno" class="form-control">
						...
						<c:forEach var="dept" items="${dept_list}" varStatus="status">
							<c:choose>
								<c:when test="${dept.deptno == item.deptno}">
									<option value="${item.deptno}" selected>${dept.dname}</option>
								</c:when>
								<c:otherwise>
		      		...
		      		for (int i=0; i<deptList.size(); i++) {
		      			Department deptItem = deptList.get(i);

						String option = null;

						// 학과 목록의 일련번호와 교수 데이터에 저장된 학과번호가 동일할 경우
						if (deptItem.getDeptno() == item.getDeptno()) {
							option = "<option value='%d' selected>%s</option>";
						} else {
							option = "<option value='%d'>%s</option>";
						}

		      			String tag = String.format(option, deptItem.getDeptno(), deptItem.getDname());
		      			out.print(tag);	
		      		}
		      	%> --%>
					</select>
